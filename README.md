...
1. IDE 默认会寻找项目根目录的 `project.config.json`。若小程序项目有单独的目录，则需要配置后使用。打开 VSCode 的 **首选项** — **设置** 进入设置页，选择 **工作区** 并依次展开左侧 **扩展** — **微信小程序开发工具**，找到配置项 **Miniprogram Path**，填写小程序项目的目录名称或相对路径即可。设置完成后会在项目目录下生成 `.vscode/settings.json` 用于存储。
