...
async function getSearchResult(query: string): Promise<SearchResult[]> {
  const response = await fetch(`https://developers.weixin.qq.com/search?action=wxa_search&size=50&query=${query}&page=1&language=1&type=wxadoc&doc_type=miniprogram`);
